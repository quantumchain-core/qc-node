...
impl Mempool {
    ...
    pub fn add(&mut self, tx: Transaction) -> Result<(), MempoolError> {
        ...
        // 2. Base fee check (EIP-1559: sender's base_fee must cover protocol)
        ...
        // 4. Nonce ordering — reject if nonce already queued for sender
        let sender_queue = self.by_sender.entry(tx.from).or_default();
        if sender_queue.contains_key(&tx.nonce) {
            // Could support replacement (higher fee) here — left as TODO
            return Err(MempoolError::NonceTooLow);
        }

        // 5. Per-sender cap
        if sender_queue.len() >= self.config.per_sender_max {
        ...
        // 7. TODO: signature verification (hook into src/crypto/)
        ...
        sender_queue.insert(tx.nonce, hash);
...
mod tests {
    ...
    fn test_peek_best_ordering() {
        let mut pool = Mempool::new(MempoolConfig::default());
        // Add 3 txs with different priority fees
    ...
    fn test_base_fee_update_evicts() {
        ...
        // tx with base_fee 1500 should be evicted
        assert_eq!(pool.len(), 1);
    }
